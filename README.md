...
The browser passphrase is a full-control credential, not a viewer token. A connected client can use every family the home server advertises, including opening every Relay route with credentials held by that server. Route URIs and credentials never enter route snapshots or routine errors in the default edge mode. Font metadata is visible when enabled, but font bytes require `YAS_FONT_EXPORT=1` and remain subject to each face's OS/2 embedding restrictions. Non-loopback deployments must protect every browser transport with TLS; exposing the default plaintext WebSocket listener to an untrusted network leaks full server authority. See the complete [trust model](docs/design/yas.md#trust-model).
